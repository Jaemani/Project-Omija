...
Detailed collection steps are maintained in `docs/data-collection-playbook.md`.
The non-executing query-seed generator is:

```bash
uv run python scripts/collection_plan.py
```

It writes `out/collection_plan.json` and `out/collection_plan.md` without calling private feeds.
